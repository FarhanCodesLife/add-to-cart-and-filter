...
function renderitems(btn) {
            ...
            return items.category === btn.innerHTML}).filter((items,i)=>{
...
<div class="card-body">
<button onclick="addtocartbycatagery(${items,i})">add to cart</button>
...
function addtocartbycatagery(items,index) {

    if (addarray.includes(items[index])) {
   
     items[index].quantity += 1 
   }
    else{
     addarray.push(items[index])
   items[index].quantity = 1
     }
...
let sum = products.reduce(function (acumlator, initialvalue) {
    return acumlator + initialvalue.price
...
h1.innerHTML = `Total All price is = ${sum}`
...
function addtocart(index) {
 ...
 if (addarray.includes(products[index])) {

  products[index].quantity += 1 
}
 else{
  addarray.push(products[index])
products[index].quantity = 1
  }
  console.log(addarray);
